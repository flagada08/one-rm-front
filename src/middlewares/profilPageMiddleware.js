...
import { fetchUserData, fetchUserDataExercises } from 'src/actions/pageProfil';
...
const profilPageMiddelware = (store) => (next) => (action) => {
  ...
  const fetchDataExercises = () => {
    const API_URL = 'http://charlie-bauduin.vpnuser.lan/Apotheose/O-ne-RM/O-NE-RM/public/api/user/performances';
    const TOKEN = localStorage.getItem('token');
    axios.get(API_URL, { headers: { Authorization: `Bearer ${TOKEN}` } })
      .then((response) => {
        const { data } = response;
